...
### [COPIAR ROTEIRO PARA CONFIGURAR COM IA →](COMECE_COM_IA.md)
...
| 2. Entrar na aplicação | Execute `cd app`. Todo o código-fonte fica nessa única pasta. |
| 3. Instalar | Execute `pnpm install`. |
| 4. Configurar ambiente | Copie `environment.example` para `.env` e preencha apenas os valores da **sua** instalação. |
| 5. Preparar infraestrutura | Disponibilize um domínio HTTPS público e um banco MySQL próprios. |
| 6. Configurar Instagram | Crie seu aplicativo no painel Meta e cadastre as URLs de retorno e webhook do seu domínio. |
| 7. Executar | Rode `pnpm dev` no desenvolvimento ou faça o deploy no seu provedor. |
| 8. Conectar e testar | Entre no painel, conecte uma conta Business ou Creator, crie uma automação, aprove-a e faça um teste controlado. |
...
**[Abrir o roteiro seguro para IA](COMECE_COM_IA.md)**

## Variáveis e segurança

O arquivo [`app/environment.example`](app/environment.example) lista as variáveis necessárias sem qualquer valor real. Guarde segredos apenas no `.env` local ou no gerenciador de segredos da sua hospedagem.
...
Você pode fazer fork, criar uma branch e abrir uma pull request. As contribuições passam pela validação automática de testes e tipos. Leia **[CONTRIBUTING.md](docs/CONTRIBUTING.md)** para o fluxo de contribuição e a lista de cuidados de segurança.
